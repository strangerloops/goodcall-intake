...
class IntakeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let labelFrame = CGRect(x: 20, y: 40, width: view.frame.width, height: 100)
        let label = UILabel(frame: labelFrame)
        label.text = Config.questionForIndex(index: index)
        label.textColor = UIColor.black

        let textFieldFrame = CGRect(x: 20, y: 100, width: view.frame.width, height: 40)
        textField = UITextField(frame: textFieldFrame)

        let buttonFrame = CGRect(x: 0, y: 120, width: 100, height: 100)
